...
uint8  tempDataLen;
uint8 CommitiFlag=0;                      //通信标志位 保证部分数据只接受一次
...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
  ...
  {
    HalUARTRead (port, &ch, 1);                        //收到的数据为ch
    
//***********************串口数据处理部分*************************************//
    if((ch=='W') && (CommitiFlag==0))                 //接收确认数据为'W' 通信标识位为0
    {
      CommitiFlag=1;                                  //通信标志位置1
      HalUARTWrite(0,"W@",2);                         //回复网关'W@'
    }
    if(((ch=='M') || (ch=='C')) && (CommitiFlag==1))  //接收工作模式数据'M'或'C' 通信标志位为1
    {
       CommitiFlag=2;                                 //通信标志位置2 不再接收确认和模式数据
       Mode=ch;                                       //模式数据赋值
       Timer_Flag=1;                                  //计算总定时时间标志位置1 开启
       WaitFlag=0;                                    //串口等待标志位清零 关闭
       LinkWait=1;                                    //建网等待标志位置1 开启建网延时
       ans[2]=Mode;                                   
       HalUARTWrite(0,ans,3);                         //应答网关 'OKC'
    }
    if(ch=='R')                                       //若接收数据为'R'
    {
      SystemReset();                                  //重启
